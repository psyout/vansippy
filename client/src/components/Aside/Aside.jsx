...
const filterAndSort = (
         ...
         ({ properties }) =>
            ...
            Object.keys(properties).some(
               (key) =>
                  ...
                  properties[key]?.toString().toLowerCase().includes(q),
            ),
      )
      .filter(({ properties }) => !filterBy || properties.category === filterBy)
      .filter(
         ({ properties }) => !hoodBy || properties.neighbourhoods === hoodBy,
      );
   ...
   if (sortBy === "hours")
      return filtered.sort((a, b) =>
         hoursStr(a.properties.hours).localeCompare(
            hoursStr(b.properties.hours),
         ),
      );
   return filtered;
};
...
function Aside({
   ...
   const renderCards = sortedFeatures
      .filter(
         ({ properties }) =>
            properties?.id &&
            businesses.some((b) => (b?._id || b?.id) === properties.id),
      )
      .map(({ properties }, index) => {
         ...
         const shouldExpandFirst = Boolean(
            isFirstLoad &&
            index === 0 &&
            !selectedBusiness &&
            properties.drinks,
         ...
         const shouldExpandSelected = Boolean(
            !isFirstLoad && isSelected && properties.drinks,
         );
   ...
   if (selectedBusiness) {
      const selectedIndex = sortedFeatures.findIndex(
         ({ properties }) => properties.id === selectedBusiness,
      );
      if (selectedIndex > -1) {
   ...
   return (
      <div className="aside">
         ...
         <ul className="aside__list">
            <ResponsiveMasonry
               columnsCountBreakPoints={{ 450: 1, 690: 2, 950: 2 }}
            >
